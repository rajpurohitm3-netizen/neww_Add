...
export function UserDashboardView({ session, privateKey }: UserDashboardViewProps) {
  ...
  const [showNotifications, setShowNotifications] = useState(false);
    ...
    const openChat = (contact: any) => {
      ...
      const isVerified = sessionStorage.getItem("messages_pin_verified") === "true";
      if (!isVerified && !messagesPinVerified) {
        setActiveView("chat");
      } else {
                        ...
                        <h3 className="text-sm font-black uppercase tracking-[0.3em] mb-6 font-accent">Recent Chats</h3>
                          <div className="space-y-2">
                            {recentChats.map(chat => (
                              <button key={chat.id} onClick={() => openChat(chat)} className="w-full flex items-center gap-4 p-4 hover:bg-white/5 rounded-2xl transition-all">
                      ...
                      <div className="bg-white/[0.02] border border-white/5 rounded-[2rem] p-6">
                        <h3 className="text-sm font-black uppercase tracking-[0.3em] mb-6 font-accent">Global Directory</h3>
                          <div className="grid grid-cols-2 gap-3">
                            {profiles.map(p => (
                              <div key={p.id} onClick={() => openChat(p)} className="p-4 bg-white/[0.02] border border-white/5 rounded-2xl flex flex-col items-center gap-3 cursor-pointer hover:border-indigo-500/30">
                    ...
                    {activeView === "chat" && (
                      !messagesPinVerified ? (
                        <SecurityPin 
                          ...
                          description="Enter decryption code to view conversations"
                          onSuccess={() => {
